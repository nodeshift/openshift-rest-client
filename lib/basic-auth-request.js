...
async function getTokenFromBasicAuth (settings) {
  return new Promise((resolve, reject) => {
    ...
    const credentials = `${settings.user || settings.username}:${settings.pass || settings.password}`;
    ...
    const req = {
      ...
      strictSSL: 'insecureSkipTlsVerify' in settings ? !Boolean(settings.insecureSkipTlsVerify) : true
    };
