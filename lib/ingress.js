...
function find (client) {
  return function find (ingresName, options = {}) {
    ...
    const url =
      `${clientConfig.cluster}${client.apis.v1beta1.endpoints()
        .extensions}/namespaces/${clientConfig.context.namespace}/ingresses/${ingresName}`;
...
function create (client) {
  return function create (ingress, options = {}) {
    ...
    const url =
      `${clientConfig.cluster}${client.apis.v1beta1.endpoints()
        .extensions}/namespaces/${clientConfig.context.namespace}/ingresses`;
...
function update (client) {
  return function create (name, ingress, options = {}) {
    ...
    const url =
      `${clientConfig.cluster}${client.apis.v1beta1.endpoints()
        .extensions}/namespaces/${clientConfig.context.namespace}/ingresses/${name}`;
