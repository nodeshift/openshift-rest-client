...
const { config, Request } = require('kubernetes-client/backends/request');
...
async function openshiftClient (settings = {}) {
  ...
  const client = new Client({ backend: new Request(kubeconfig || clusterconfig.loadFromCluster()), version: '1.14' });
